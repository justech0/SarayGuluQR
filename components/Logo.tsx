...
export const Logo: React.FC<{ className?: string; variant?: 'light' | 'dark'; size?: 'md' | 'lg' }> = ({
  className,
  variant = 'light',
  size = 'md',
}) => {
  const iconColor = '#D4AF37';
  const sizeClass = size === 'lg' ? 'h-36 md:h-40' : 'h-16 md:h-20';
  ...
  return (
    <motion.div
      ...
      className={`flex flex-col items-center justify-center text-center w-full ${sizeClass} ${className}`}
    >
      {/* Rose Abstract Icon */}
      <div className="mb-3 relative drop-shadow-md h-full">
         <svg className="h-full w-auto max-h-none" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
